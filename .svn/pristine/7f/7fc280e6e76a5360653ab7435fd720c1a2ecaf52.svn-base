...
public class QuantityView extends LinearLayout implements View.OnClickListener {
    ...
    private double maxQuantity = Integer.MAX_VALUE, minQuantity = Integer.MAX_VALUE;
    ...
    public interface OnQuantityChangeListener {
        void onQuantityChanged(double oldQuantity, double newQuantity, boolean programmatically);
    ...
    private void init(AttributeSet attrs, int defStyle) {
        ...
        maxQuantity = a.getFloat(R.styleable.QuantityView_qv_maxQuantity, Integer.MAX_VALUE);
        minQuantity = a.getFloat(R.styleable.QuantityView_qv_minQuantity, 0);
    ...
    public void onClick(View v) {
        if (v == mButtonAdd) {
            if (quantity + 1 > maxQuantity) {
                ...
                double oldQty = quantity;
                quantity = MathUtils.add(quantity, 1);
                mTextViewQuantity.setText(MathUtils.doubleTrans(quantity));
            ...
            if (quantity <= minQuantity) {
                ...
                if (quantity < 1 && quantity > 0) {
                    ...
                    quantity = MathUtils.sub(quantity, 1);
                }
                mTextViewQuantity.setText(MathUtils.doubleTrans(quantity));
                if (onQuantityChangeListener != null)
                    onQuantityChangeListener.onQuantityChanged(oldQty, quantity, false);
            ...
            et.setText(MathUtils.doubleTrans(quantity));
            ...
            dialog.getButton(AlertDialog.BUTTON_POSITIVE).setOnClickListener(new OnClickListener() {
                @Override
                public void onClick(View v) {
                    final String newQuantity = et.getText().toString();
                    if (!isValidNumber(newQuantity)) {
                        Toast.makeText(getContext(), getResources().getString(R.string.invalid_num_prompt), Toast.LENGTH_LONG).show();
                        return;
                    }
                    final double intNewQuantity = Double.parseDouble(newQuantity);
                    Log.d(VIEW_LOG_TAG, "newQuantity " + intNewQuantity + " max " + maxQuantity);
                    if (intNewQuantity > maxQuantity) {
                        Toast.makeText(getContext(), getResources().getString(R.string.max_upper_limit_prompt) + maxQuantity, Toast.LENGTH_LONG).show();
                    } else if (intNewQuantity < minQuantity) {
                        Toast.makeText(getContext(), getResources().getString(R.string.min_lower_limit_prompt) + minQuantity, Toast.LENGTH_LONG).show();
                    } else {
                        if (InputUtils.isDecimal(newQuantity)) {
                            ...
                            materialDialog.setPositiveButton("确认", new OnClickListener() {
                                        @Override
...
//            }
            this.quantity = MathUtils.roundUp(newQuantity, 2);
            mTextViewQuantity.setText(MathUtils.doubleTrans(quantity));
        } else {
            if (onQuantityChangeListener != null) onQuantityChangeListener.onLimitReached();
        }
