package com.frxs.receipt.rest.service.apkUpdate;
...
import com.frxs.receipt.widget.ProgressAlertDialog;
...
//            var2.setDataAndType(uriForFile, var1.getContentResolver().getType(uriForFile));
            ...
            Uri apkUri = FileProvider.getUriForFile(var1, var1.getApplicationContext().getPackageName() + ".fileprovider", var0);
            var2.addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION);//添加这一句表示对目标应用临时授权该Uri所代表的文件
            var2.setDataAndType(apkUri, "application/vnd.android.package-archive");
        }else{
